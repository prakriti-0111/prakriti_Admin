...
import { subCategoryList } from 'actions/admin/subCategory.actions';
import { categoryList } from 'actions/admin/category.actions';
import { orderStore } from 'actions/admin/order.actions';
import { ADMIN_RESET_PRODUCT_LIST } from '../../../actionTypes/admin/product.types';
import { ADMIN_RESET_SUB_CATEGORY_LIST } from '../../../actionTypes/admin/subCategory.types';
import { cartStore, cartList } from 'actions/admin/cart.actions';
import { ADMIN_CART_RESET, ADMIN_CART_RESET_LIST } from '../../../actionTypes/admin/cart.types';
import { ADMIN_RESET_ORDERS } from '../../../actionTypes/admin/orders.types';
...
class OrderCreatePage extends React.Component {
    ...
    componentDidUpdate() {
        if (this.state.cart_actionCalled) {
            ...
            if (this.state.order_createSuccess) {
                ...
                this.props.dispatch({
                    type: ADMIN_RESET_ORDERS
                });
                this.props.dispatch({
                    type: ADMIN_CART_RESET_LIST
                });
                this.props.navigate('/admin/my-order');
            } else {
    ...
    handleCategoryChange = (event) => {
        ...
        if (isEmpty(event.target.value)) {
            this.props.dispatch({
                type: ADMIN_RESET_SUB_CATEGORY_LIST
            })
        } else {
        ...
        return (
            <MainCard title="Create Order">
                <Box sx={{ flexGrow: 1, m: 0.5 }} className='ratn-dialog-inner'>
                    <Grid container spacing={2} columnSpacing={{ xs: 1, sm: 2, md: 2 }} className='tax-input loans_view '>
                        ...
                        <Grid item xs={12} md={6}>
                            <Stack spacing={1} direction="row" className='ratn-footer-buttons distributor-order' justifyContent="flex-end">
                                <LoadingButton
                                ...
                                {
                                    !submitting ?
                                        <Button variant="outlined" className='close-button' onClick={() => this.props.navigate('/admin/my-order')}>Cancel</Button>
                                        : null
                                }
                            </Stack>
                        </Grid>
                        ...
                        <DialogContent>
                            ...
                            <Box sx={{ flexGrow: 1, m: 0.5 }}>
                                <Grid container spacing={2}>
                                    ...
                                    <Grid item xs={productFormValues.product_type != "material" ? 12 : 12} md={productFormValues.product_type != "material" ? 1 : 3}>
                                        <TextField
                                            label="Quantity"
                                            variant="outlined"
                                            fullWidth
...
const mapStateToProps = (state) => ({
    ...
    cartItems: state.admin.cart.items,
    unitList: state.superadmin.unit.items,
    cart_actionCalled: state.admin.cart.actionCalled,
    cart_createSuccess: state.admin.cart.createSuccess,
    cart_deleteSuccess: state.admin.cart.deleteSuccess,
    cart_successMessage: state.admin.cart.successMessage,
    cart_errorMessage: state.admin.cart.errorMessage,
    order_actionCalled: state.admin.orders.actionCalled,
    order_createSuccess: state.admin.orders.createSuccess,
    order_successMessage: state.admin.orders.successMessage,
    order_errorMessage: state.admin.orders.errorMessage,
    categoryList: state.admin.category.items,
    subCategoryList: state.admin.subCategory.items,
});
