...
class PurchaseProductsPage extends Component {
  ...
  getPurchasePartyMeta = (item) => {
    ...
    const rowName = this.normalizeValue(
      item.supplier_name || item.purchase_from_name || item.user_name,
    );
    ...
    const adminMatch = (this.state.adminList || []).find((admin) => {
      ...
      return (
        (rowId && adminId && rowId === adminId) ||
        (rowName && (rowName === adminName || rowName === adminCompany))
      );
    });
    if (adminMatch) {
      return {
    ...
    const supplierMatch = (this.state.supplierList || []).find((supplier) => {
      ...
      return (
        (rowId && supplierId && rowId === supplierId) ||
        (rowName && (rowName === supplierName || rowName === supplierCompany))
      );
    });
    ...
    return {
      party_name:
        item.supplier_name ||
        item.purchase_from_name ||
        (supplierMatch ? supplierMatch.name : ""),
    };
  };
