...
import {
  returnOrderView,
  returnOrderAssign,
} from "actions/superadmin/returnOrder.actions";
...
class ReturnOrderViewPage extends React.Component {
  ...
  handleSubmit = async () => {
    if (!this.formValidate()) {
      ...
      let res = await returnOrderAssign(
        this.state.returnOrder.id,
        this.state.formValues
      );
  ...
  handleStatusSubmit = async () => {
    ...
    let res = await returnOrderUpdateStatus(this.state.returnOrder.id, {
    ...
    return (
      <MainCard
          ...
          <>
            <Grid container spacing={2} className='loans_view p_view'>
              ...
              returnOrder.status != "picked_up" ? (
                <Grid item xs={12} md={2} className='create-input'>
                  <Button variant='contained' onClick={this.handleAssign}>
                    Assign
                  </Button>
                </Grid>
              ) : null}
        ...
        <Dialog
          ...
          fullWidth
          maxWidth='xs'>
          <DialogTitle>Return Order Assign</DialogTitle>
          <DialogContent>
        ...
        <Dialog
          ...
          fullWidth
          maxWidth='xs'>
          <DialogTitle>Return Order Status</DialogTitle>
          <DialogContent>
            <DialogContentText></DialogContentText>
            <Box sx={{ flexGrow: 1, m: 0.5 }}>
              <Grid container spacing={2}>
                ...
                <Grid item xs={12}>
                  <Stack spacing={1} direction='row' justifyContent='flex-end'>
                    ...
                    <Button
                      variant='outlined'
                      onClick={this.handleStatusDialogClose}>
                      Cancel
                    </Button>
                  </Stack>
                </Grid>
...
export default withSnackbar(
  withRouter(connect(mapStateToProps, mapDispatchToProps)(ReturnOrderViewPage))
);
