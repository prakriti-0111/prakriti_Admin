...
class RetailerViewPage extends React.Component {
  ...
  componentDidUpdate(previousProps) {
    ...
    if (this.state.v_actionCalled) {
      if (this.state.v_createSuccess) {
        this.props.enqueueSnackbar(this.state.v_successMessage, {
          variant: "success",
        });
        this.setState({
          processing: false,
        ...
        this.props.enqueueSnackbar(this.state.v_errorMessage, {
          variant: "error",
        });
        this.setState({
          processing: false,
    ...
    if (this.state.r_actionCalled) {
      if (this.state.r_createSuccess) {
        this.props.enqueueSnackbar(this.state.r_successMessage, {
          variant: "success",
        });
        this.props.dispatch({
        ...
        this.props.enqueueSnackbar(this.state.r_errorMessage, {
          variant: "error",
        });
        this.props.dispatch({
          type: SUPERADMIN_RESET_RETAILER,
  ...
  handleSaleNow = () => {
    this.props.navigate(
      getUserDashboardRoute(getRoleName(this.state.auth)) + "/stocks"
    );
  };
  ...
  handleReviewSubmit = () => {
    if (this.state.rating <= 0) {
      return this.props.enqueueSnackbar("Please give rating.", {
        variant: "error",
      });
    }
    let data = {
  ...
  render() {
    ...
    const reviewTotalPage = Math.ceil(
      this.state.review_total / this.state.reviewQueryParams.limit
    );
    const showAsTotalRetailer =
      this.props.query.get("total_retailer") == 1 ? false : true;
        ...
        <Dialog
          ...
          <DialogTitle>
            ...
            {this.state.pay_by_invoice
              ? "/ " + this.state.pay_by_invoice.invoice_number
              : null}
          </DialogTitle>
          <DialogContent>
                ...
                <Grid item xs={12} md={3} className='create-input'>
                  <FormControl fullWidth>
                    ...
                    <Select
                      ...
                      {!this.state.pay_by_invoice &&
                      (!retailer || parseFloat(retailer.due_amount) == 0) ? (
            ...
            <Box sx={{ flexGrow: 1, m: 0.5 }}>
              <Grid container spacing={2}>
                <Grid
                  item
                  xs={12}
                  md={visitformValues.type == "Not Interested" ? 12 : 6}
                  ...
                  <FormControl fullWidth>
                    ...
                    <Select
                      ...
                      label='Type'
                      onChange={(event) =>
                        this.updateVisitFormValue(event.target.value, "type")
                      }>
                      <MenuItem value='Interested'>Interested</MenuItem>
                      <MenuItem value='Not Interested'>Not Interested</MenuItem>
                      <MenuItem value='Interested in future'>
                        Interested in future
                      </MenuItem>
                    </Select>
                  </FormControl>
                </Grid>
                {visitformValues.type == "Interested in future" ||
                visitformValues.type == "Interested" ? (
                  <Grid
                    item
                    xs={12}
                    md={6}
                    ...
                    <LocalizationProvider dateAdapter={AdapterDayjs}>
                      <DatePicker
                        ...
                        inputFormat='DD/MM/YYYY'
                        onChange={(newValue) =>
                          this.updateVisitFormValue(newValue, "date")
                        }
                        renderInput={(params) => (
                ...
                <Grid item xs={12} className='create-input'>
                  <TextareaAutosize
                    ...
                    onChange={(event) =>
                      this.updateVisitFormValue(event.target.value, "notes")
                    }
                  />
                </Grid>
                <Grid item xs={12}>
...
export default withSnackbar(
  withRouter(connect(mapStateToProps, mapDispatchToProps)(RetailerViewPage))
);
