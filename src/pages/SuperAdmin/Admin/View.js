...
class AdminViewPage extends React.Component {
  ...
  render() {
    ...
    return (
      <MainCard title="Admin Details" secondary={<Button variant="contained" onClick={() => this.props.navigate(-1)}>Back</Button>}>
        {
          !admin ?
...
const mapDispatchToProps = dispatch => ({
  dispatch,
  actions: bindActionCreators({ adminFetch, paymentStore, salesList, getNotifiactions, stocksList, categoryList, subCategoryList }, dispatch)
});

export default withSnackbar(withRouter(connect(mapStateToProps, mapDispatchToProps)(AdminViewPage)));
