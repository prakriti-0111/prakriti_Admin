...
import {
  ...
  orderUpdateStatus,
  updateOrderProducts,
} from "actions/superadmin/order.actions";
...
class OrderView extends React.Component {
  ...
  loadViewData = () => {
    orderViewRaw(this.props.params.id).then((res) => {
      if (res.data.success) {
        this.setState({
          order: res.data.data,
    ...
    if (res.data.success) {
      this.props.navigate(
        getUserDashboardRoute(getRoleName(this.state.auth)) +
          "/sales/create?order_id=" +
          this.state.order.id +
          "&from_order_price=" +
          this.state.from_order_price +
          "&all_added=" +
          res.data.data.all_added
      );
    } else {
      this.setState({
        processing: false,
  ...
  handleSubmit = async () => {
    ...
    let res = await orderUpdateStatus(this.state.order.id, {
  ...
  render() {
    const { order, action_products, is_updating, receiving_product } =
      this.state;
    let statusArr = [];
    if (order) {
        ...
        {!order ? (
          ...
          <>
                          ...
                          <TableRow key={i}>
                            ...
                            <TableCell>
                              <img
                                ...
                                onClick={() => this.handleImageClick(row.image)}
                              />
                            </TableCell>
                                ...
                                <>
                                  {receiving_product.products.map((c, ck) => (
                                    <input
                                      ...
                                      name='certificate_no'
                                      onChange={(e) =>
                                        this.handleCertificate(e, ck)
                                      }
                                      className={
                                        "custom_input" +
                                        (c.certificate_no_err
                                          ? " error_input"
                                          : "")
                                      }
                                      style={{
                            ...
                            <TableCell sx={{ paddingTop: "0px" }}>
                              {row.status == "pending" ? (
                                            ...
                                            {this.hasChecked(i) ? (
                                              <div
                                                ...
                                                <input
                                                  type='text'
                                                  value={
                                                    action_products["item_" + i]
                                                      .materials[key].weight
                                                  }
                                                  name='weight'
                                                  onChange={(e) =>
                                                    this.handleMaterial(
                                ...
                                <>
                                  {this.isRecieving(row) ? (
                                    <>
                                      {receiving_product.products.map(
                                        (x, k) => (
                                          <Table
                                            className='table-bordered input_table'
                                            key={k}>
                                            <TableHead className='ratn-table-header'>
                                              <TableRow>
                                                <TableCell>Name</TableCell>
                              ...
                              {row.size_name ? (
                                <>
                                  ...
                                  row.product_type != "material" ? (
                                    <select
                                      onChange={(e) =>
                                        this.handleSizeChange(e, row)
                                      }
                                      name='size_id'
                                      value={row.size_id}>
                                      {row.cart_data.size_materials.map(
                                        (p, k) => (
                                          <option value={p.size_id} key={k}>
                                            {p.size_name}
                                          </option>
                                        )
                                      )}
                            ...
                            <TableCell>
                              ...
                              {row.status == "on_process" ? (
                                  ...
                                  {this.isRecieving(row) ? (
                                    <>
                                      ...
                                      <div>
                                        <button
                                          onClick={() =>
                                            this.handleCancelRecive()
                                          }
                                          className='receive_cancel_btn'>
                                          Cancel
                                        </button>
                                      </div>
                                    </>
                                ...
                                <>
                                  {row.status == "received" ? (
                                    <div>
                                      <button className='receive_success_btn'>
                                        Received
                                      </button>
                                    </div>
                                  ) : null}
                ...
                <Grid item xs={12} md={12} style={{ paddingBottom: "15px" }}>
                  <Stack
                    ...
                    <Button
                      ...
                      type='button'
                      onClick={this.handleProductConfirmSubmit}>
                      Send
                    </Button>
                  </Stack>
                </Grid>
            ...
            <Dialog
              ...
              fullWidth
              maxWidth='xs'>
              <DialogTitle>Receive Product</DialogTitle>
              <DialogContent>
                <DialogContentText></DialogContentText>
                <Box sx={{ flexGrow: 1, m: 0.5 }}>
                  <Grid container spacing={2}>
                    ...
                    <Grid item xs={12}>
                      <Stack
                        ...
                        <Button
                          ...
                          type='button'
                          disabled={this.state.processing}
                          onClick={this.handleReceiveSubmitProceed}>
                          {this.state.processing ? "Processing" : "Submit"}
                        </Button>
                        <Button
                          variant='outlined'
                          onClick={this.handleReceiveDialogClose}>
                          Cancel
                        </Button>
                      </Stack>
                    </Grid>
...
export default withSnackbar(
  withRouter(connect(mapStateToProps, mapDispatchToProps)(OrderView))
);
