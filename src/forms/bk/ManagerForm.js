...
import {SUPERADMIN_RESET_MANAGER} from '../../actionTypes/superadmin/manager.types';
...
import { managerCreate, managerUpdate } from 'actions/superadmin/manager.actions';
...
class ManagerForm extends React.Component {
        ...
        if(this.state.actionCalled){
            if(this.state.isCreateFrom){
                if(this.state.createSuccess){
                    ...
                    this.props.dispatch({
                        type: SUPERADMIN_RESET_MANAGER
                    });
                    this.props.navigate('/super-admin/managers');
                }else{
                    this.setState({
                        submitting: false
                    })
    ...
    handleSubmit = async () => {
        ...
        if(!hasErr){
            ...
            if(this.state.isCreateFrom){
                this.props.actions.managerCreate(postData);
            }else{
                this.props.actions.managerUpdate(this.state.formData.id, postData);
            }
        }
    }
...
const mapStateToProps = (state) => ({
    actionCalled: state.superadmin.manager.actionCalled,
    createSuccess: state.superadmin.manager.createSuccess,
    editSuccess: state.superadmin.manager.editSuccess,
    successMessage: state.superadmin.manager.successMessage,
    errorMessage: state.superadmin.manager.errorMessage,
...
export default withSnackbar(withRouter(connect(mapStateToProps, mapDispatchToProps)(ManagerForm)));
