...
import { salesStore, salesUpdate } from 'actions/admin/sales.actions';
import { stocksProductList, stocksProducDetails } from 'actions/admin/stocks.actions';
import { materialPriceProductPriceInfo } from 'actions/admin/materialPrice.actions';
import { distributorList } from 'actions/admin/distributor.actions';
import { productList } from 'actions/admin/product.actions';
...
import { ADMIN_GET_STOCK_PRODUCT_DETAILS_RESET } from '../../actionTypes/admin/stocks.types';
...
import { saleCartDelete, saleCartListRaw } from 'actions/admin/saleCart.actions';
...
class SaleForm extends React.Component {
    ...
    handleUserChange = (event) => {
        ...
        let m = _.filter(this.state.distributorList, { id: event.target.value });
        let user_gst_no = '';
        if (m.length) {
            user_gst_no = m[0].gst;
        }
    ...
    handleCategoryChange = (event) => {
        ...
        this.props.dispatch({
            type: ADMIN_GET_STOCK_PRODUCT_DETAILS_RESET
        })
                ...
                <Grid container spacing={2} className='tax-input loans_view p_view'>
                    ...
                    <Grid item xs={6} className='create-input'>
                        <FormControl fullWidth error={formErros.user_id}>
                            <InputLabel>Ditributor</InputLabel>
                            <Select
                            ...
                            >
                                <MenuItem value="">select ditributor</MenuItem>
                                {
...
const mapStateToProps = (state) => ({
    ...
    productList: state.admin.product.items,
    actionCalled: state.admin.sales.actionCalled,
    createSuccess: state.admin.sales.createSuccess,
    editSuccess: state.admin.sales.editSuccess,
    successMessage: state.admin.sales.successMessage,
    errorMessage: state.admin.sales.errorMessage,
    productPriceInfo: state.admin.materialPrice.productPriceInfo,
    stockProductList: state.admin.stocks.productList,
    stockProductDetails: state.admin.stocks.productDetails,
