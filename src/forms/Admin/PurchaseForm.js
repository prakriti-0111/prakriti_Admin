...
import {
  ...
  purchaseReturn,
} from "actions/admin/purchase.actions";
...
class PurchaseForm extends React.Component {
  ...
  handleDeleteConfirm = () => {
    const formValues = {
      ...
      products: [...this.state.formValues.products],
    };
  ...
  handleCalculateMainPrice = (products = this.state.formValues.products) => {
    let formValues = {
      ...this.state.formValues,
      products: [...products],
      ...this.getCalculatedMainPriceValues(products),
    };
    this.setState({
      formValues: formValues,
    });
  ...
  handleSubmit = () => {
    ...
    if (!hasErr && formValues.products.length) {
      const calculatedValues = this.getCalculatedMainPriceValues(
        formValues.products,
      );
      this.setState({
        submitting: true,
      ...
      if (this.state.isCreateFrom) {
        this.props.actions.purchaseStore({
          ...this.state.formValues,
          ...calculatedValues,
        });
      } else {
        this.props.actions.purchaseUpdate(this.state.formData.id, {
          ...this.state.formValues,
          ...calculatedValues,
        });
      }
      ...
      <Box sx={{ flexGrow: 1, m: 0.5 }} className="ratn-dialog-inner">
        ...
        <Grid container spacing={2} className="loans_view tax-input">
          <Grid item xs={12} className="p-add-product create-input">
            <h3 className="p_heading_list mb-0 mt-0">
              Product List{" "}
            ...
            {this.state.productDialog ? (
              <Box sx={{ flexGrow: 1, m: 0.5 }}>
                ...
                >
                  ...
                  <Grid
                    item
                    xs={productFormValues.product_type == "in_house" ? 3 : 3}
                  >
            ...
            >
              ...
              >
                <TableHead className="ratn-table-header p_view">
                  <TableRow>
                    {!this.state.isCreateFrom ? <TableCell></TableCell> : null}
                ...
                <TableBody>
                      ...
                      >
                        ...
                        <TableCell>
                          {item.materials.map((m, key) => (
                            ...
                            >
                              {m.material_name} &nbsp;{" "}
                              {!this.state.isCreateFrom && m.quantity ? (
                                <>
                                  {m.avl_qty -
                                    parseInt(
                  ...
                  {formValues.products.length > 0 ? (
                    <TableRow>
                      <TableCell
                        colSpan={this.state.isCreateFrom ? 8 : 9}
                      ></TableCell>
                      <TableCell>
                        <b>
                          Sub Total
