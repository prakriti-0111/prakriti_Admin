...
import { updateEditProfile } from 'actions/employee/profile.actions';
import { withSnackbar } from 'notistack';
import LoadingButton from '@mui/lab/LoadingButton';
import withRouter from 'src/helpers/withRouter';
import {EMPLOYEE_PROFILE_RESET} from '../../actionTypes/employee/profile.types';
            ...
            <Box sx={{ flexGrow: 1, m: 0.5 }} className='ratn-dialog-inner'>
                <Grid container spacing={2} className='tax-input loans_view p_view '>
                    ...
                    <Grid item xs={12} className='create-input'>
                        ...
                        {
                            !submitting ? 
                            <Button variant="outlined" className='close-button' onClick={() => this.props.navigate('/employee/dashboard') }>Cancel</Button>
                            : null
                        }
                        </Stack>
                    </Grid>
...
const mapStateToProps = (state) => ({
    auth: state.auth,
    actionCalled: state.employee.profile.actionCalled,
    editProfileSuccess: state.employee.profile.editProfileSuccess,
    successMessage: state.employee.profile.successMessage,
    errorMessage: state.employee.profile.errorMessage,
	auth: state.auth
});
