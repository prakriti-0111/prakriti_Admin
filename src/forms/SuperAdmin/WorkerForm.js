...
import {SUPERADMIN_RESET_WORKER} from '../../actionTypes/superadmin/worker.types';
...
import { workerCreate, workerUpdate } from 'actions/superadmin/worker.actions';
...
class WorkerForm extends React.Component {
        ...
        if(this.state.actionCalled){
            if(this.state.isCreateFrom){
                if(this.state.createSuccess){
                    ...
                    this.props.dispatch({
                        type: SUPERADMIN_RESET_WORKER
                    });
                    this.props.navigate(getUserDashboardRoute(getRoleName(this.state.auth)) + '/workers');
                }else{
                    this.setState({
                        submitting: false
                    })
    ...
    handleSubmit = async () => {
        ...
        if(!hasErr){
            ...
            if(this.state.isCreateFrom){
                this.props.actions.workerCreate(postData);
            }else{
                this.props.actions.workerUpdate(this.state.formData.id, postData);
            }
        }
    }
        ...
        return (
            <Box sx={{ flexGrow: 1, m: 0.5 }}>
                ...
                <Grid container spacing={2}>
                    <Grid item xs={8} className='create-input'>
                        <TextField  
                            label="Name" 
                            variant="outlined"
                            fullWidth
                    ...
                    <Grid item xs={12}>
                        ...
                        {
                            !submitting ? 
                            <Button variant="outlined" className='close-button' onClick={() => this.props.navigate(getUserDashboardRoute(getRoleName(this.state.auth)) + '/admins') }>Cancel</Button>
                            : null
                        }
                        </Stack>
                    </Grid>
...
const mapStateToProps = (state) => ({
    actionCalled: state.superadmin.worker.actionCalled,
    createSuccess: state.superadmin.worker.createSuccess,
    editSuccess: state.superadmin.worker.editSuccess,
    successMessage: state.superadmin.worker.successMessage,
    errorMessage: state.superadmin.worker.errorMessage,
...
export default withSnackbar(withRouter(connect(mapStateToProps, mapDispatchToProps)(WorkerForm)));
