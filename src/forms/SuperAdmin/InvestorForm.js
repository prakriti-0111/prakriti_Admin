...
import { investorCreate, investorUpdate } from 'actions/superadmin/investor.actions';
...
class InvestorForm extends React.Component {
    ...
    loadUserName = async(id) => {
        let res = await getNextUserName('investor', id);
        if(res.data.success){
            this.setState({
                formValues: {
        ...
        if(this.state.actionCalled){
            if(this.state.isCreateFrom){
                if(this.state.createSuccess){
                    ...
                    this.props.navigate(getUserDashboardRoute(getRoleName(this.state.auth)) + '/investors');
                }else{
                    this.setState({
                        submitting: false
                    })
    ...
    handleSubmit = async () => {
        ...
        if(!hasErr){
            ...
            if(this.state.isCreateFrom){
                this.props.actions.investorCreate(postData);
            }else{
                this.props.actions.investorUpdate(this.state.formData.id, postData);
            }
        }
    }
                ...
                <Grid container spacing={2} className='loans_view p_view'>
                    ...
                    <Grid item xs={12}>
                        ...
                        <Grid item xs={3} className="admin-buttons">
                            ...
                            <Grid item xs={12} md={6} className="admin-buttons">
                    ...
                    <Grid item xs={12}>
                        ...
                        {
                            !submitting ? 
                            <Button variant="outlined" className='close-button' onClick={() => this.props.navigate(getUserDashboardRoute(getRoleName(this.state.auth)) + '/investors') }>Cancel</Button>
                            : null
                        }
                        </Stack>
                    </Grid>
...
const mapStateToProps = (state) => ({
    actionCalled: state.superadmin.investor.actionCalled,
    createSuccess: state.superadmin.investor.createSuccess,
    editSuccess: state.superadmin.investor.editSuccess,
    successMessage: state.superadmin.investor.successMessage,
    errorMessage: state.superadmin.investor.errorMessage,
...
export default withSnackbar(withRouter(connect(mapStateToProps, mapDispatchToProps)(InvestorForm)));
