...
import {Box, TextField, Button, Grid, Stack, TextareaAutosize, FormControl, InputLabel, Select, MenuItem, FormLabel, FormGroup, FormControlLabel, Checkbox  } from '@mui/material';
...
import { distributorCreate, distributorUpdate } from 'actions/superadmin/distributor.actions';
import DeleteIcon from '@mui/icons-material/Delete';
import CloudUploadIcon from '@mui/icons-material/CloudUpload';
import _ from 'lodash';
import {getRoleName, getUserDashboardRoute, isAdmin} from 'src/helpers/helper';
...
class DistributorForm extends React.Component {
    ...
    loadProfile = async() => {
        ...
        if(res.data.success){
            ...
            this.setState({
                ...
                this.loadSelectList(this.state.formValues);
            });
    ...
    loadUserName = async(id) => {
        let res = await getNextUserName('distributor', id);
        if(res.data.success){
            this.setState({
                formValues: {
                    ...this.state.formValues,
    ...
    handleDefaultChange = (event, key) => {
        ...
        if(key == 'expense' || key == 'expense_action'){
            vl = event.target.checked ? vl : '';
        }
        this.updateFormValues(vl, key);
    }
    ...
    handleSubmit = async () => {
        ...
        if(!hasErr){
            ...
            if(this.state.isCreateFrom){
                this.props.actions.distributorCreate(postData);
            }else{
                this.props.actions.distributorUpdate(this.state.formData.id, postData);
            }
        }
    }
    ...
    render() {
        const { formValues, formErros, submitting, isCreateFrom } = this.state;
            ...
            <Box sx={{ flexGrow: 1, m: 0.5 }} className='ratn-dialog-wrapper'>
                    ...
                    <Grid item xs={2} className='create-input'>
                        <FormControl fullWidth>
                            ...
                            <Select
                                ...
                                value={formValues.own}
                                fullWidth
                                label="Own Distributor"
                                onChange={(event) => this.handleDefaultChange(event, 'own')}
                            >
                    ...
                    <Grid item xs={12} md={4} className='create-input'>
                        <FormControl component="fieldset">
                            ...
                            <FormGroup aria-label="position" row>
                                <FormControlLabel
                                    value="1"
                                    control={<Checkbox checked={this.state.formValues.expense} name="expense" onChange={(event) => this.handleDefaultChange(event, 'expense')} />}
                                    label="Expense"
                                    labelPlacement="end"
                                />
                                <FormControlLabel
                                    value="1"
                                    control={<Checkbox checked={this.state.formValues.expense_action} name="expense_action" onChange={(event) => this.handleDefaultChange(event, 'expense_action')} />}
                ...
                <Grid container spacing={2} className='loans_view p_view'>
                    ...
                    <Grid item xs={3}>
                        ...
                        {
                            !submitting ? 
                            <Button variant="outlined" className='close-button' onClick={() => this.props.navigate(getUserDashboardRoute(getRoleName(this.state.auth)) + '/distributors') }>Cancel</Button>
                            : null
                        }
                        </Stack>
                    </Grid>
...
export default withSnackbar(withRouter(connect(mapStateToProps, mapDispatchToProps)(DistributorForm)));
