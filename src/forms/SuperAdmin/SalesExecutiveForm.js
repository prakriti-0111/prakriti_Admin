...
import { salesExecutiveCreate, salesExecutiveUpdate } from 'actions/superadmin/salesExecutive.actions';
...
class SalesExecutiveForm extends React.Component {
    ...
    componentDidMount(){
        ...
        this.props.actions.distributorList({all: 1, own: 1});
    ...
    static getDerivedStateFromProps(props, state){
        ...
        if(props.isEditProfile !== state.isEditProfile){
            update.isEditProfile = props.isEditProfile;
        }
        ...
        if(this.state.actionCalled){
            if(this.state.isCreateFrom){
                if(this.state.createSuccess){
                    ...
                    this.props.navigate(getUserDashboardRoute(getRoleName(this.state.auth)) + '/sales-executive');
                }else{
                    this.setState({
                        submitting: false
                    })
                ...
                if(this.state.editSuccess){
                    ...
                    if(this.state.isEditProfile){
                        ...
                        if(this.state.profile_image){
                            updateData.image = this.state.profile_base64;
                        }else{
                            updateData.image = this.state.existing_profile_image;
                        }
                        this.props.dispatch({
    ...
    handleSubmit = async () => {
        ...
        if(!hasErr){
            ...
            if(this.state.profile_image){
                ...
                this.setState({
                    profile_base64: postData.profile_image
                })
            }else{
                this.setState({
                    profile_base64: ""
            ...
            if(this.state.isCreateFrom){
                this.props.actions.salesExecutiveCreate(postData);
            }else{
                this.props.actions.salesExecutiveUpdate(this.state.formData.id, postData);
            }
        }
    }
    ...
    render() {
        const { formValues, formErros, submitting, isEditProfile, isCreateFrom } = this.state;
        ...
        return (
            <Box sx={{ flexGrow: 1, m: 0.5 }} className='ratn-dialog-wrapper'>
                ...
                <Grid container spacing={2} className='loans_view p_view'>
                    ...
                    {!isCreateFrom && <Grid item md={2} xs={12} className='create-input'>
                        <TextField  
                            label="Created By" 
                            variant="outlined"
                            fullWidth
                    ...
                    <Grid item md={3} xs={12} className='create-input'>
                        <FormControl fullWidth error={formErros.parent_id}>
                            ...
                            <Select
                                ...
                                fullWidth
                                label="Distributor"
                                onChange={(event) => this.handleDefaultChange(event, 'parent_id')}
                                disabled={isEditProfile}
                                inputProps={{ className: "non_disable_text" }}
                            >
                                <MenuItem value=""></MenuItem>
                                {
                                    this.state.distributorList.map((item, index) => (
                                        <MenuItem value={item.id} key={index}>{item.name+"  (" +item.company_name + ")"}</MenuItem>
                                    ))
                                }
                            </Select>
                        </FormControl>
                    </Grid>
                    <Grid item xs={12} md={7} className='create-input'>
                        <TextField  
                            label="Full Address" 
                            variant="outlined"
                            fullWidth
                    ...
                    <Grid item md={2.4} xs={12} className='create-input'>
                ...
                <div className='custom-container ml-10'>
                        ...
                        <div className='custom-col-8'>
                            <div className='all-image-wrapper'>
                                <div className='all-single-image'>
                                            ...
                                            <>
                                                {
                                                    ...
                                                    <button className='close_img' onClick={() => this.deleteExistingDocument(0)} disabled={isEditProfile}>x</button>
                                                    :
                                                    <button className='close_img' onClick={() => this.deleteDocument(0)} disabled={isEditProfile}>x</button>
                                                }
                                            </>
                                            : null
                                        }
                                    ...
                                    <Button variant="contained" className='image-button' component="label" sx={{maxWidth: '260px'}} endIcon={<CloudUploadIcon />} disabled={isEditProfile}>
                                        Documents
                                        <input
                                        name="documents"
                                        hidden
                                        ...
                                        <div className='all-single-image' key={index}>
                                            <div>
                                                ...
                                                <>
                                                    {
                                                    item.type == "existing" ?
                                                    <button className='close_img' onClick={() => this.deleteExistingDocument(index+1)} disabled={isEditProfile}>x</button>
                                                    :
                                                    <button className='close_img' onClick={() => this.deleteDocument(index+1)} disabled={isEditProfile}>x</button>
                                                    }
                                                </>
                                                : null
                                                }
                    ...
                    {/*<Grid item xs={12}>
                        <Grid container spacing={2}>
                            <Grid item md={3} xs={12} className="admin-buttons">
                                {
...
const mapStateToProps = (state) => ({
    actionCalled: state.superadmin.salesExecutive.actionCalled,
    createSuccess: state.superadmin.salesExecutive.createSuccess,
    editSuccess: state.superadmin.salesExecutive.editSuccess,
    successMessage: state.superadmin.salesExecutive.successMessage,
    errorMessage: state.superadmin.salesExecutive.errorMessage,
...
SalesExecutiveForm.defaultProps = {
    isEditProfile: false
}

export default withSnackbar(withRouter(connect(mapStateToProps, mapDispatchToProps)(SalesExecutiveForm)));
